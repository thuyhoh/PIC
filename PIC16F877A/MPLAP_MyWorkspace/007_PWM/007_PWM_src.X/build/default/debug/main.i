...
uint32_t PWM_FREQ = 0;
float PWM_PERIOD= 0;
uint8_t PRESCALE_VAL = 16;
void PWM_SetFreq(uint32_t hz);
void PWM_SetDutyCycles(uint8_t duty);
...
void PWM_SetFreq(uint32_t f_hz)
{
# 65 "main.c"
    ...
    int PR2_value = (int)((PWM_PERIOD * 2000000) / (4 * PRESCALE_VAL) - 1);
    PR2 = PR2_value;

}
void PWM_SetDutyCycles(uint8_t duty)
{
    float duty_period = PWM_PERIOD*duty/100;

    int tempreg = (int)((duty_period * 2000000) / 16);

    CCPR1L = tempreg >> 2;
    CCP1CONbits.CCP1X = (tempreg & 0b00000001);
    CCP1CONbits.CCP1Y = (tempreg & 0b00000010);
